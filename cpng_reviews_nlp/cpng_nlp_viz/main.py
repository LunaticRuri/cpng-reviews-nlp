...
class EventHandler:
    ...
    def get_product_img_url(product_id, product_name):
        ...
        box = soup.find("li", {"class": "basicList_item__2XT81"})

        if not box:
            return False

        code_str = box.find('a')['data-nclick']
        code = re.findall(r'\d+', code_str)[-1]
        root_code = code[:-4]
    ...
    def update_product_desc(target_id, target_name):
        img_url = EventHandler.get_product_img_url(target_id, target_name)
        if not img_url:
