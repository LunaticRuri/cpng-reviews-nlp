...
def download_review_by_product(product_id):
    ...
    # category가 내부 카테고리가 아니라 외부 배치 기준
    ...
    for ratings in range(1, 6):
        ...
        while char_count < MAX_CHAR_COUNT:
            review_page_url = f'https://www.coupang.com/vp/product/reviews?productId={product_id}&page={str(target_page)}' \
                              f'&size=10&sortBy=ORDER_SCORE_ASC&ratings={str(ratings)}&q=&viRoleCode=3&ratingSummary=true'
            try:
...
def download_stories(product_list):
    threads = min(MAX_THREADS, len(product_list))

    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        executor.map(download_review_by_product, product_list)


if __name__ == '__main__':
    print("concurrent.futures:")
    t0 = time.time()
    download_stories(['7958974', '130911332'])
    t1 = time.time()
    print(f"{t1 - t0} seconds to download {len([1, 2])} stories.")

    print("normal method:")
    t0 = time.time()
    download_review_by_product('7958974')
    download_review_by_product('130911332')
    t1 = time.time()
    print(f"{t1 - t0} seconds to download {len([1, 2])} stories.")
